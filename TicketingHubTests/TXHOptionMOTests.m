...
@interface TXHOptionMOTests : XCTestCase
...
- (void) testCreationFromTXHOption {
    XCTAssertTrue(self.option, @"Option should exist");
    XCTAssertTrue(self.venueMO, @"Venue should exist");

    TXHOptionMO *optionMO = [TXHOptionMO createWithOption:self.option forVenue:self.venueMO];

    XCTAssertTrue(optionMO, @"Should have created an option managed object");

    XCTAssert(optionMO.weekdayIndexValue == 1, @"Weekday set");
    XCTAssertEqualObjects(optionMO.duration, self.option.duration, @"Should be equal");
    XCTAssertEqualObjects(optionMO.timeString, self.option.timeString, @"Time should be set");
    XCTAssertTrue(optionMO.venue, @"Venue relationship object should exist");
